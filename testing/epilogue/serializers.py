...
class CustomerSerializer( CustomerUpdateMixin , serializers.ModelSerializer):
    ...
    class Meta:
        model = Customer
        fields = ["email" , "first_name" , "last_name" , "mobile" , "age" , "weight" , "height", "lifestyle" , "objective" , "id", "gender" , "body_type" , "food_cat" ,"weight_type" , "height_type" , "work_pref" , "level" , "diseases" , "injuries" , "reasons", "new_latest_activity", "current_level", "level_in_use", "create_on"]
...
class CustomerDietFavouriteSerializer(serializers.ModelSerializer):
    ...
    customer_calendar = serializers.PrimaryKeyRelatedField( queryset = CustomerIsoWeek.objects.all())
